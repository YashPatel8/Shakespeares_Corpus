...
#Corpus the main structure that tm uses for storing and manipulating text documents.
#There are two types VCorpus (Volatile Corpus) and PCorpus (Permanent Corpus)
#The main difference between these two implementations is that
#the former holds the documents as R objects in memory whereas
#the latter deals with documents that are stored outside of R environment. 
#When we are constructing our Corpus object, we need to provide a source
#There are three types of such sources, DirSource, VectorSource and DataFrameSource.
#We will use DirSource to import the three text files that we just downloaded and
#using DirSource is the only way to import files from the user system
